...
Deno.serve(async(request:Request)=>{
 ...
 const {data:membership}=await caller.from("memberships").select("id").eq("organization_id",organizationId).eq("user_id",auth.user.id).eq("role","admin").eq("status","active").maybeSingle();if(!membership)return reply(403,{error:"Only administrators can create portal access."});
