-- Admin operates within their organization without impersonating a technician.
-- Retain branch/tenant checks, evidence append-only rules, qualification and financial locks.
...
begin
 ...
 select jsonb_build_object(
  ...
  'can_record',exists(select 1 from public.technician_profiles t where t.id=v_i.technician_id and t.organization_id=p_organization_id and t.active and (t.user_id=auth.uid() or app_private.is_admin(p_organization_id))) and app_private.has_permission(p_organization_id,v_i.branch_id,'inspection.perform'),
  'can_reassign',app_private.is_admin(p_organization_id) and v_i.status='in_progress',
  'recording_on_behalf',app_private.is_admin(p_organization_id) and exists(select 1 from public.technician_profiles t where t.id=v_i.technician_id and t.organization_id=p_organization_id and t.user_id<>auth.uid()),
...
begin
  ...
  if v_job.status not in ('assigned', 'ready', 'paused') then
    raise exception 'Only assigned, ready or paused jobs can be started' using errcode = '22023';
  end if;

  if app_private.is_admin(v_job.organization_id) then
    select t.* into v_technician from public.technician_profiles t
    join public.job_assignments a on a.technician_id=t.id
    where a.job_id=v_job.id and a.assignment_kind='primary' and a.unassigned_at is null
      and t.organization_id=v_job.organization_id and t.active;
    if not found then raise exception 'Assign an active primary technician before starting the timer' using errcode='23514'; end if;
  else
  ...
  if v_job.required_qualification_code is not null and not app_private.technician_has_qualification(
    v_job.organization_id, v_technician.user_id, v_job.required_qualification_code
  ) then
  ...
  insert into audit.events(organization_id,actor_id,action,entity_type,entity_id,metadata)
  values(v_job.organization_id,auth.uid(),'job.timer_started','job',v_job.id,
    jsonb_build_object('technician_id',v_technician.id,'recorded_on_behalf',v_technician.user_id<>auth.uid()));
  return v_job;
end;
$function$;
...
begin
  ...
  if app_private.is_admin(v_job.organization_id) then
    if (select count(*) from public.labor_entries where job_id=v_job.id and ended_at is null)<>1 then
      raise exception 'Exactly one active timer is required to stop this job' using errcode='23514';
    end if;
    select t.* into v_technician from public.technician_profiles t
    join public.labor_entries l on l.technician_id=t.id
    where l.job_id=v_job.id and l.ended_at is null and t.organization_id=v_job.organization_id;
  ...
  values (v_job.organization_id, auth.uid(), 'job.timer_stopped', 'job', v_job.id,
    jsonb_build_object('outcome', p_outcome, 'note', nullif(trim(p_note), ''), 'technician_id', v_technician.id, 'recorded_on_behalf', v_technician.user_id<>auth.uid()));
  return v_job;
end;
$function$;
...
begin
  ...
  if (v_session.staff_user_id is distinct from auth.uid() and not app_private.is_admin(v_session.organization_id)) or not app_private.has_permission(v_session.organization_id,v_session.branch_id,'payment.receive') then raise exception 'Only the owning cashier or an administrator can close this register' using errcode='42501'; end if;
  if v_session.status<>'open' or p_counted_close is null or p_counted_close<0 then raise exception 'Open session and non-negative count are required' using errcode='22023'; end if;
  ...
  insert into audit.events(organization_id,actor_id,action,entity_type,entity_id,metadata)
  values(v_session.organization_id,auth.uid(),'cash_session.closed','cash_session',v_session.id,
    jsonb_build_object('cashier_user_id',v_session.staff_user_id,'recorded_on_behalf',v_session.staff_user_id<>auth.uid(),
      'counted_close',v_session.counted_close,'expected_close',v_session.expected_close,'variance',v_session.variance));
  return v_session;
end; $function$;
