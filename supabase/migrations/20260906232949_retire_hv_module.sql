-- Retire the HV operating module without deleting evidence or relaxing fault restrictions.
-- Routine maintenance, inspection results, diagnostics and battery-health records remain available.
begin;

create or replace function app_private.is_maintenance_check(p_rules jsonb)
returns boolean language sql immutable set search_path = '' as $$
  select coalesce(p_rules->>'capability', '') not in ('hv', 'soh')
    and nullif(trim(p_rules->>'qualification'), '') is null;
$$;
revoke all on function app_private.is_maintenance_check(jsonb) from public, anon, authenticated;

create or replace function app_private.enforce_maintenance_scope()
...
begin
  case tg_table_name
    when 'jobs' then
      v_allowed := v_row->>'safety_class' in ('normal', 'ev_aware')
        and nullif(trim(v_row->>'required_qualification_code'), '') is null;
    when 'service_template_tasks' then
      v_allowed := coalesce(v_row->'result_schema'->>'safety_class', 'normal') in ('normal', 'ev_aware')
        and nullif(trim(v_row->>'required_qualification_code'), '') is null
        and coalesce(v_row->>'required_permission', '') <> 'hv_permit.authorize';
    when 'inspection_check_definitions' then
      v_allowed := app_private.is_maintenance_check(v_row->'rules');
    when 'inspection_checklist_tasks' then
      v_allowed := app_private.is_maintenance_check(v_row->'snapshot'->'rules');
    when 'branch_capabilities' then
      v_allowed := v_row->>'capability_code' <> 'HV_SERVICE' or v_row->>'status' <> 'active';
    when 'resources' then
      v_allowed := coalesce(v_row->'capabilities'->>'hv_ready', 'false') <> 'true';
    when 'membership_permissions' then
      v_allowed := v_row->>'permission_code' <> 'hv_permit.authorize' or v_row->>'allowed' = 'false';
    else v_allowed := false;
  end case;
  if v_allowed is not true then
    raise exception 'Outside maintenance scope' using errcode = '23514';
  end if;
  return new;
end;
$$;
revoke all on function app_private.enforce_maintenance_scope() from public, anon, authenticated;

create trigger maintenance_scope before insert or update of safety_class, required_qualification_code on public.jobs
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of result_schema, required_qualification_code, required_permission on public.service_template_tasks
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of rules on public.inspection_check_definitions
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of snapshot on public.inspection_checklist_tasks
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of capability_code, status on public.branch_capabilities
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of capabilities on public.resources
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update of permission_code, allowed on public.membership_permissions
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update on public.hv_work_permits
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update on public.hv_permit_checks
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update on public.qualification_types
for each row execute function app_private.enforce_maintenance_scope();
create trigger maintenance_scope before insert or update on public.technician_qualifications
for each row execute function app_private.enforce_maintenance_scope();
...
revoke all on function public.record_hv_permit_evidence(uuid, text, text, uuid, text, numeric, text, date, text, text, jsonb, text) from public, anon, authenticated;
...
language sql stable security definer set search_path='' as $$
 ...
 where d.active and app_private.is_maintenance_check(d.rules) and (d.organization_id is null or d.organization_id=c.organization_id)
