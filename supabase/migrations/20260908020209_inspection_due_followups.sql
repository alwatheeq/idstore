-- Link due follow-ups to the original checklist task; preserve all historic evidence.
alter table public.vehicle_recommendations add column source_task_id uuid references public.inspection_checklist_tasks(id);
create unique index vehicle_recommendations_open_task on public.vehicle_recommendations(source_task_id)
 where source_task_id is not null and status in ('open','scheduled');
create index vehicle_recommendations_followup_due on public.vehicle_recommendations(organization_id,vehicle_id,due_date)
...
declare
 ...
 v_due date; v_followup uuid; v_prev public.inspection_check_results%rowtype; v_finding uuid; v_vehicle uuid; v_status text;
begin
 ...
 -- Retired inputs from older clients are ignored, never stored or required.
 p_data:=p_data-'measurement'-'unit'-'criteria'-'evidence';
 ...
 if p_action='assign' then
  if v_i.checklist_generated_at is not null and not app_private.is_admin(v_i.organization_id) then raise exception 'Only an administrator can change assignment after checklist generation' using errcode='42501'; end if;
  ...
  if p_action='exception' then
   ...
   select * into v_tp from public.technician_profiles where id=v_i.technician_id and organization_id=v_i.organization_id and active and (user_id=auth.uid() or app_private.is_admin(v_i.organization_id));
   if not found then raise exception 'Only the assigned technician or an administrator can record results' using errcode='42501'; end if;
   if nullif(v_t.snapshot->'rules'->>'qualification','') is not null and
    (nullif(v_t.snapshot->'rules'->>'procedure_ref','') is null or not app_private.technician_has_qualification(v_i.organization_id,v_tp.user_id,v_t.snapshot->'rules'->>'qualification')) then raise exception 'A current verified qualification and vehicle procedure are required'; end if;
   ...
   if nullif(p_data->>'due_date','') is not null then
    if p_data->>'due_date' !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' then raise exception 'Invalid follow-up due date' using errcode='22023'; end if;
    v_due:=(p_data->>'due_date')::date;
   end if;
   if nullif(p_data->>'recommendation','') is not null and p_data->>'recommendation' not in ('none','monitor','service','repair','replace','diagnose') then raise exception 'Invalid recommendation' using errcode='22023'; end if;
   if nullif(p_data->>'urgency','') is not null and p_data->>'urgency' not in ('immediate','soon','routine') then raise exception 'Invalid urgency' using errcode='22023'; end if;
   ...
   select v_i.organization_id,v_i.branch_id,v_i.id,v_t.definition_id,coalesce(max(sequence),0)+1,
    ...
    jsonb_build_object('check',v_t.snapshot->>'label_en','check_ar',v_t.snapshot->>'label_ar'),
   ...
   if v_result='fail' or v_due is not null or p_data->>'recommendation' in ('monitor','service','repair','replace','diagnose') then
    select vehicle_id into v_vehicle from public.repair_orders where id=v_i.repair_order_id;
    insert into public.vehicle_recommendations(organization_id,branch_id,vehicle_id,finding_id,source_task_id,description,severity,due_date)
     values(v_i.organization_id,v_i.branch_id,v_vehicle,v_finding,v_t.id,concat_ws(' · ',v_t.snapshot->>'label_en',nullif(p_data->>'finding',''),nullif(p_data->>'recommendation','none')),case p_data->>'urgency' when 'immediate' then 'safety_stop' when 'soon' then 'red' else 'amber' end,v_due)
    on conflict(source_task_id) where source_task_id is not null and status in ('open','scheduled')
    do update set description=excluded.description,severity=excluded.severity,
      finding_id=coalesce(excluded.finding_id,public.vehicle_recommendations.finding_id),
      due_date=coalesce(excluded.due_date,public.vehicle_recommendations.due_date),updated_at=clock_timestamp()
    returning id into v_followup;
    insert into audit.events(organization_id,actor_id,action,entity_type,entity_id,metadata)
     values(v_i.organization_id,auth.uid(),'inspection.followup_saved','vehicle_recommendation',v_followup,
      jsonb_build_object('inspection_id',v_i.id,'task_id',v_t.id,'due_date',v_due));
   end if;
  end if;
  insert into public.inspection_check_results(organization_id,branch_id,task_id,attempt,result,details,technician_id,recorded_by,inspection_item_id)
   values(v_i.organization_id,v_i.branch_id,v_t.id,v_attempt,v_result,(p_data-'task_id'-'expected_attempt') || jsonb_build_object('recorded_on_behalf',(p_action='record' and v_tp.user_id is distinct from auth.uid())::text),case when p_action='record' then v_i.technician_id else null end,auth.uid(),v_item.id);
 ...
 insert into audit.events(organization_id,actor_id,action,entity_type,entity_id,metadata) values(v_i.organization_id,auth.uid(),'inspection.'||p_action,'inspection',v_i.id,jsonb_build_object('task_id',v_t.id,'attempt',v_attempt,'count',v_count,'technician_id',case when p_action='assign' then v_tp.id else v_i.technician_id end,'recorded_on_behalf',p_action='record' and v_tp.user_id is distinct from auth.uid(),'previous_assignment',case when p_action='assign' then v_i.assignment else null end,'previous_technician_id',case when p_action='assign' then v_i.technician_id else null end));
 return jsonb_build_object('id',v_i.id,'action',p_action);
end;
$function$;

create or replace function public.reschedule_vehicle_followup(p_id uuid,p_due_date date,p_updated_at timestamptz)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_r public.vehicle_recommendations%rowtype;
begin
 select * into v_r from public.vehicle_recommendations where id=p_id for update;
 if not found or not (app_private.has_permission(v_r.organization_id,v_r.branch_id,'inspection.perform') or app_private.has_permission(v_r.organization_id,v_r.branch_id,'repair_order.manage')) then raise exception 'Not authorized' using errcode='42501'; end if;
 if v_r.status not in ('open','scheduled') or p_updated_at is distinct from v_r.updated_at then raise exception 'Follow-up changed; refresh before saving' using errcode='40001'; end if;
 update public.vehicle_recommendations set due_date=p_due_date,updated_at=clock_timestamp() where id=v_r.id;
 insert into audit.events(organization_id,actor_id,action,entity_type,entity_id,metadata)
 values(v_r.organization_id,auth.uid(),'followup.rescheduled','vehicle_recommendation',v_r.id,jsonb_build_object('previous_due_date',v_r.due_date,'due_date',p_due_date));
 return jsonb_build_object('id',v_r.id);
end;
$$;
revoke all on function public.reschedule_vehicle_followup(uuid,date,timestamptz) from public,anon;
grant execute on function public.reschedule_vehicle_followup(uuid,date,timestamptz) to authenticated;
