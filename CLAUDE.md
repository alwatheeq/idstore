...
- **Applied to prod:** also `0004_software_updates.sql`, `0005_vehicle_model_images.sql`, `0006_order_concerns.sql`, `0007_multi_branch.sql` (branch metadata, `admin_users.home_branch_id`+`can_access_all_branches` with the existing owner seeded super, `invoices.branch_id` backfill, branch-scoped RLS via `is_super_admin()`/`admin_home_branch()`/`can_access_branch(b)`), `0008_inventory.sql` (shared catalog `inventory_items`+`suppliers`; per-branch `inventory_stock` + `inventory_movements` ledger with a `apply_inventory_movement` trigger that keeps stock in sync; branch-scoped RLS on stock/movements), `0009_purchase_orders.sql` (`purchase_orders` + `purchase_order_lines`, branch-scoped RLS; receiving posts receive-movements through the inventory ledger), and `0010_order_line_inventory.sql` (`service_order_lines.inventory_item_id` + `issued_qty` — a part line can link a catalog item and auto-issue/return branch stock). No pending migrations.
...
## Phase status
Phase 1 (operational core) complete: CRM, Service Orders + intake/inspection, Invoicing, Dashboard. Phase 2: Customer Portal, Software-update tracking done; Notifications (WhatsApp) remain. Phase 3 (in progress): **multi-branch DONE** (Phase A — branch CRUD, active-branch context/switcher, branch-scoped queries + RLS). Remaining Phase 3: inventory — **Phase B DONE** (catalog/suppliers/per-branch stock/movements receive·issue·adjust·transfer·count/low-stock/valuation; `src/features/inventory/`, `/inventory` + `/inventory/:id` + `/suppliers`); **Phase C DONE** (purchase orders: `src/features/purchasing/`, `/purchase-orders` + `/purchase-orders/:id`, draft→ordered→received; "Receive all" posts receive-movements); **Phase D DONE** (order↔inventory: a service-order part line can pick a catalog item → auto-fills price + issues that branch's stock via the inventory ledger; edits reconcile, delete returns). **Phase 3 inventory + multi-branch COMPLETE.** **Accounting (reporting) DONE** — read-only `/accounting`: cash-basis revenue + month trend, outstanding receivables, payments-by-method, purchases (cash out); period presets + custom range; branch-scoped; no migration (`src/features/accounting/`). Remaining: Phase 2 WhatsApp notifications. UI uses the "Volt Instrument" design system (see `src/index.css` tokens).
