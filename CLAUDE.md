...
- **Applied to prod:** also `0004_software_updates.sql`, `0005_vehicle_model_images.sql`, `0006_order_concerns.sql`, and `0007_multi_branch.sql` (branch metadata, `admin_users.home_branch_id`+`can_access_all_branches` with the existing owner seeded super, `invoices.branch_id` backfill, and the branch-scoped RLS rewrite via `is_super_admin()`/`admin_home_branch()`/`can_access_branch(b)`). No pending migrations.
...
## Phase status
Phase 1 (operational core) complete: CRM, Service Orders + intake/inspection, Invoicing, Dashboard. Phase 2: Customer Portal, Software-update tracking done; Notifications (WhatsApp) remain. Phase 3 (in progress): **multi-branch DONE** (Phase A — branch CRUD, active-branch context/switcher, branch-scoped queries + RLS). Remaining Phase 3: inventory — **Phase B** (catalog/suppliers/per-branch stock/movements/low-stock/valuation), **Phase C** (purchase orders), **Phase D** (order↔inventory: pick parts from catalog → auto-decrement stock). Then accounting. UI uses the "Volt Instrument" design system (see `src/index.css` tokens).
