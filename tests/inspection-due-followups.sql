...
declare
 ...
 followup uuid; stamp timestamptz; version_no bigint; payload jsonb; ws jsonb; assignment jsonb; snapshot_before jsonb;
begin
 ...
 snapshot_before:=jsonb_build_object('id',definition,'code','TEST_CHECK','label_en','Test check','label_ar','فحص تجريبي','rules','{"evidence_required":true,"criteria":"Legacy criteria","unit":"mm"}'::jsonb);
 ...
 payload:=jsonb_build_object('task_id',task,'expected_attempt',0,'result','pass','due_date',((now() at time zone 'Asia/Amman')::date+7)::text,'urgency','soon','recommendation','monitor','recorded_on_behalf','false');
 ...
 perform public.inspection_workflow(inspection,'record',payload||'{"measurement":"discard","unit":"discard","criteria":"discard","evidence":"discard"}');
 if exists(select 1 from public.inspection_check_results where task_id=task and details ?| array['measurement','unit','criteria','evidence']) then raise exception 'Retired fields persisted'; end if;
 if exists(select 1 from public.inspection_items where inspection_id=inspection and measurement_json ? 'measurement') then raise exception 'Legacy measurement persisted'; end if;
 ...
 -- Failure still needs a finding/recommendation/urgency, but no retired fields.
 perform public.inspection_workflow(inspection,'record',payload||'{"expected_attempt":2,"result":"fail","finding":"Needs repair","recommendation":"repair","urgency":"routine","retest_note":"Additional check"}');
 -- Admin catalog cannot re-enable removed requirements.
 perform public.configure_inspection_check(org,jsonb_build_object('code','TEST_SIMPLE','category','identity','label_en','Simple check','label_ar','فحص مبسط','rules','{"groups":["general"],"evidence_required":true,"criteria":"ignored","unit":"mm"}'::jsonb));
 if exists(select 1 from public.inspection_check_definitions where organization_id=org and code='TEST_SIMPLE' and rules ?| array['evidence_required','criteria','unit']) then raise exception 'Retired catalog settings persisted'; end if;
 -- Follow-up stays one open item across retests and records the chosen date.
 if (select count(*) from public.vehicle_recommendations where source_task_id=task and status in ('open','scheduled'))<>1 then raise exception 'Retest duplicated reminder'; end if;
 select id,updated_at into followup,stamp from public.vehicle_recommendations where source_task_id=task and status='open';
 if (select due_date from public.vehicle_recommendations where id=followup) is distinct from ((now() at time zone 'Asia/Amman')::date+7) then raise exception 'Due date not stored'; end if;
 perform set_config('request.jwt.claim.sub',outsider::text,true);
 begin perform public.reschedule_vehicle_followup(followup,current_date+10,stamp); raise exception 'Cross-tenant reschedule allowed'; exception when insufficient_privilege then null; end;
 perform set_config('request.jwt.claim.sub',admin_user::text,true);
 perform public.reschedule_vehicle_followup(followup,current_date+10,stamp);
 begin perform public.reschedule_vehicle_followup(followup,current_date+11,stamp-interval '1 second'); raise exception 'Stale reschedule allowed'; exception when serialization_failure then null; end;
 if not exists(select 1 from audit.events where entity_id=followup and action='followup.rescheduled' and actor_id=admin_user) then raise exception 'Reschedule not audited'; end if;
 ...
 perform public.inspection_workflow(inspection,'record',payload||'{"expected_attempt":3,"retest_note":"Technician verification","recorded_on_behalf":"true"}');
 if not exists(select 1 from public.inspection_check_results where task_id=task and attempt=4 and recorded_by=tech_user and details->>'recorded_on_behalf'='false') then raise exception 'Technician attribution regression'; end if;
...
$$;
select 'Due date, deduplication, rescheduling and permission tests passed; all fixtures rolled back' as result;
rollback;
