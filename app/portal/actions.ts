...
export async function requestPaymentLink(formData:FormData){const invoiceId=formText(formData,"invoiceId");if(!invoiceId)redirect(routeMessage("/portal","error","Choose a payable invoice."));try{const supabase=await createClient();const{error}=await supabase.rpc("portal_request_payment_link",{p_invoice_id:invoiceId});if(error)throw error;}catch(error){redirect(routeMessage("/portal","error",operationError(error,"The payment-link request could not be queued.")));}revalidatePath("/portal");redirect(routeMessage("/portal","created","A secure payment-link request has been queued."));}
