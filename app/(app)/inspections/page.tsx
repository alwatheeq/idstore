...
import { RecordRemovalForm } from "@/components/record-removal-form"; import { RecordAction } from "@/components/record-action"; import Link from "next/link";
import { InspectionServiceSelection } from "@/components/inspection-service-selection";
...
import { cancelInspection, createInspection, saveInspectionWorkflow } from "./actions";

type Query = { cancel?: string; new?: string; order?: string; inspection?: string; setup?: string; created?: string; error?: string };
export default async function InspectionsPage({ searchParams }: { searchParams: Promise<Query> }) {
  ...
  const cancellable = staff.role === "admin" ? visible.find(i => i.id === query.cancel && ["draft", "in_progress"].includes(i.status)) : undefined; const activeOrders = new Set((inspections ?? []).filter(i => ["draft", "in_progress"].includes(i.status)).map(i => i.repair_order_id));
  const eligible = (orders ?? []).filter(o => !activeOrders.has(o.id) && (!staff.selectedBranchId || o.branch_id === staff.selectedBranchId));
  const selected = query.inspection ? (inspections ?? []).find(i => i.id === query.inspection)
    : visible.find(i => i.repair_order_id === query.order && i.status !== "cancelled");
  ...
  return <LocalizedContent>{<>
    ...
    {cancellable ? <RecordRemovalForm action={cancelInspection} fields={{ inspectionId: cancellable.id, updatedAt: cancellable.updated_at }} title="Cancel inspection" name={cancellable.repair_order?.ro_number ?? ""} description="Cancel this unfinished inspection. Recorded checks and findings remain in its history." back="/inspections" /> : null}{staff.role === "admin" && query.setup === "1" && catalog ? <InspectionCatalog checks={catalog.catalog} models={models ?? []} saveAction={saveInspectionWorkflow} /> : null}
    ...
    {selected && workspace?.inspection ? <section className="panel inspection-workspace" id="inspection-workspace">
      ...
      {selected.status === "completed" ? <><InspectionServiceSelection inspectionId={selected.id} organizationId={staff.organizationId} orderType={selected.repair_order?.order_type ?? "maintenance"} canSelect={staff.role === "admin" || workspace.can_record} /><div className="iw-section"><Link className="button" href={`/work-orders?order=${selected.repair_order_id}#order-items`}>Return to work order</Link></div></> : null}
    </section> : null}
    {!visible.length ? <EmptyState icon={ClipboardCheck} title="No inspections yet" description="Start an inspection from an active repair order and record each vehicle condition check." action={<Link className="button" href={eligible.length ? "/inspections?new=1#new-inspection" : "/work-orders?new=1#new-work-order"}>{eligible.length ? "Start inspection" : "Open work order"}</Link>} /> : <section className="panel inspection-ledger"><div className="panel-header"><div className="panel-title">Inspection ledger</div></div><div className="data-scroll"><table className="data-table"><thead><tr><th>Repair order</th><th>Customer & vehicle</th><th>Branch</th><th>Status</th><th></th></tr></thead><tbody>{visible.map(i => <tr key={i.id}><td className="mono">{i.repair_order?.ro_number}</td><td><div className="cell-main">{i.repair_order?.vehicle?.model?.name} · <bdi>{i.repair_order?.vehicle?.registration_no ?? i.repair_order?.vehicle?.vin}</bdi></div><div className="cell-sub">{i.repair_order?.customer?.display_name}</div></td><td>{i.repair_order?.branch?.city}</td><td><StatusPill label={i.status} tone={i.status === "completed" ? "green" : "blue"} /></td><td><div className="record-actions"><RecordAction kind={["draft", "in_progress"].includes(i.status) ? "edit" : "view"} href={`/inspections?inspection=${i.id}#inspection-workspace`} />{staff.role === "admin" && ["draft", "in_progress"].includes(i.status) ? <RecordAction kind="delete" label="Cancel inspection" href={`/inspections?cancel=${i.id}#confirm-record-action`} /> : null}</div></td></tr>)}</tbody></table></div></section>}
  </>}</LocalizedContent>;
}
