...
export default async function RecordsPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  if (query.vehicle && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(query.vehicle)) return <RecordFeedback error="The selected vehicle is unavailable."/>;
  ...
  if (query.vehicle && !vehicles?.some(v => v.id === query.vehicle)) return <RecordFeedback error="The selected vehicle is unavailable."/>;
  ...
  return <>
    ...
    <section className="panel" id="record-upload"><div className="panel-header"><div><div className="panel-title">Attach to an available record</div><div className="panel-subtitle">These compact upload forms bind type and identifier server-side.</div></div></div><div className="record-targets">{targetGroups.flatMap((group) => group.items.map((item) => <form action={uploadEvidence} className="record-target" key={`${group.type}:${item.id}`}><input type="hidden" name="linkedType" value={group.type} /><input type="hidden" name="linkedId" value={item.id} /><div><span>{group.label}</span><strong>{item.label}</strong></div><select name="classification" defaultValue="confidential" aria-label="Classification"><option value="internal">Internal</option><option value="confidential">Confidential</option><option value="restricted">Restricted</option></select><input name="file" type="file" accept=".pdf,.json,.txt,.jpg,.jpeg,.png,.webp,.bin" aria-label="Evidence file" required /><button className="button compact" type="submit">Attach</button></form>))}</div></section>
  </>;
}
