...
export default async function BranchesPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  const [{ data: branches, error: branchError }, { data: activeOrders }] = await Promise.all([
    supabase
      .from("branches")
      .select("id, code, legal_name, display_name, city, address_json, phone, whatsapp, email, status, branch_capabilities(capability_code, status), warehouses(name)")
  ...
  return <>
    ...
    {!branches?.length ? <EmptyState icon={Building2} title="No branches yet" description="Create the first service center before adding customers, vehicles or work orders." action={staff.role === "admin" ? <Link className="button primary" href="/branches?new=1#new-branch">Create first branch</Link> : undefined} /> : <>
      <div className="cards-grid">{branches.map((branch, index) => {
        ...
        return <article className={`branch-card ${index === 0 ? "selected" : ""}`} key={branch.id}><div className="branch-card-head"><div><div className="branch-code">{branch.code}</div><h3>{branch.display_name}</h3><p>{branch.city}, Jordan</p></div><StatusPill label={hvCapable ? "HV capable" : "General service"} tone={hvCapable ? "green" : "gray"} /></div><div className="branch-contact-list"><span><MapPin /> {branchAddress(branch.address_json) || "Address required"}</span><span><Phone /> {branch.phone || "Phone required"}</span><span><MessageCircle /> {branch.whatsapp || "WhatsApp required"}</span></div><div className="branch-stats"><div><span>Active jobs</span><strong className="mono">{orderCounts.get(branch.id) ?? 0}</strong></div><div><span>Status</span><strong style={{ fontSize: 14 }}>{branch.status}</strong></div></div>{staff.role === "admin" ? <Link className="branch-edit-link" href={`/branches?edit=${branch.id}#branch-contacts`}><Pencil /> Contacts</Link> : null}</article>;
      })}</div>
      <section className="panel" style={{ marginTop: 20 }}><div className="panel-header"><div><div className="panel-title">Network configuration</div><div className="panel-subtitle">Live branch contacts and warehouse readiness</div></div></div><div className="data-scroll"><table className="data-table branch-directory"><thead><tr><th>Branch</th><th>Address</th><th>Warehouse</th><th>Phone</th><th>WhatsApp</th><th>HV service</th><th>Status</th></tr></thead><tbody>{branches.map((branch) => {
        const hvCapable = branch.branch_capabilities.some((capability) => capability.capability_code === "HV_SERVICE" && capability.status === "active");
        return <tr key={branch.id}><td><div className="cell-main">{branch.display_name}</div><div className="cell-sub">{branch.code} · {branch.legal_name}</div></td><td>{branchAddress(branch.address_json) || "Required"}</td><td>{branch.warehouses[0]?.name ?? "Not configured"}</td><td className="mono">{branch.phone ?? "Required"}</td><td className="mono">{branch.whatsapp ?? "Required"}</td><td><StatusPill label={hvCapable ? "enabled" : "disabled"} tone={hvCapable ? "green" : "gray"} /></td><td><StatusPill label={branch.status} tone={branch.status === "active" ? "green" : "gray"} /></td></tr>;
