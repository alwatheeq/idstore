...
import { addCustomerContact, createCustomer, provisionCustomerPortal, recordCustomerConsent } from "./actions";
...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  const [{ data: branches }, { data: customers, error }] = await Promise.all([
    ...
    supabase
      .from("customers")
      .select("id, display_name, customer_type, status, created_at, preferred_branch:branches(display_name, city), customer_contacts(kind, value, is_primary), customer_addresses(city, is_primary), consents(purpose, channel, state, policy_version, source, recorded_at), vehicle_ownerships(vehicle_id), invoices(grand_total, created_at), customer_accounts(status)")
