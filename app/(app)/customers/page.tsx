...
type PageQuery = { new?: string; portal?: string; manage?: string; tab?: string; created?: string; error?: string };
...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  return <>
    ...
    {query.manage ? (() => {
      ...
      const customerTab = query.tab === "history" || query.tab === "access" ? query.tab : "overview";
      return <section className="panel operation-form customer-workspace" id="customer-controls">
        ...
        <nav className="customer-tabs" aria-label="Customer sections">
          ...
          <Link className={`customer-tab ${customerTab === "history" ? "active" : ""}`} aria-current={customerTab === "history" ? "page" : undefined} href={`/customers?manage=${customer.id}&tab=history#customer-controls`}>History</Link>
          ...
          <aside className="customer-profile-rail">
            ...
            <div className="customer-vehicle-list">
              {customer.vehicle_ownerships.length ? customer.vehicle_ownerships.map((ownership) => {
                ...
                return <article key={ownership.id}><div className="customer-vehicle-icon"><CarFront aria-hidden="true" /></div><div><strong>{ownership.vehicle?.model?.name ?? "VW ID"}</strong><span className="mono">{plateOrVin ?? "No plate / VIN"}</span><small>{ownership.relationship === "owner" ? "Owner" : ownership.relationship ?? "Owner"}</small>{ownership.verified_at ? <StatusPill label="verified" tone="green" /> : null}</div></article>;
