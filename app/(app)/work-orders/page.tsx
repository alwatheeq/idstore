  ...
  return <>
    ...
    {!workOrders?.length ? <EmptyState icon={ClipboardList} title="No work orders open" description={setupReady ? "Open the first work order when a vehicle arrives for service." : "A branch, customer and registered vehicle are required first."} action={setupReady ? <Link className="button primary" href="/work-orders?new=1#new-work-order">Open first work order</Link> : undefined} /> : <div className="stack">
      ...
      <section className="panel"><div className="panel-header"><div><div className="panel-title">Technician job ledger</div><div className="panel-subtitle">Assignment, safety classification, cause/correction and interruption evidence</div></div></div><div className="data-scroll"><table className="data-table workshop-job-ledger"><thead><tr><th>Job</th><th>Work order</th><th>Safety</th><th>Plan / actual</th><th>Primary technician</th><th>Status</th><th>Evidence</th><th>Dispatch or timer</th></tr></thead><tbody>{(jobs ?? []).map((job) => {
        ...
        const action = mine && job.status === "in_progress" ? <div className="stack"><div className="inline-actions">{hvLink}{[{ value: "qc", label: "Send QC" }, { value: "completed", label: "Complete" }].map((outcome) => <form action={finishJob} key={outcome.value}><input type="hidden" name="jobId" value={job.id} /><input type="hidden" name="version" value={job.version} /><input type="hidden" name="outcome" value={outcome.value} /><button className={`button compact ${outcome.value === "completed" ? "primary" : ""}`} type="submit">{outcome.label}</button></form>)}</div><form action={interruptJob} className="assign-control"><input type="hidden" name="jobId" value={job.id}/><input type="hidden" name="version" value={job.version}/><select name="interruptionType" aria-label="Interruption type"><option value="pause">Pause</option><option value="blocked">Block</option></select><input name="reason" aria-label="Interruption reason" placeholder="Required reason" required/><button className="button compact" type="submit">Record</button></form></div>
          : mine && ["assigned", "ready"].includes(job.status) ? <div className="inline-actions">{hvLink}<form action={startJob}><input type="hidden" name="jobId" value={job.id} /><input type="hidden" name="version" value={job.version} /><button className="button compact primary" type="submit">Start timer</button></form></div>
          : mine && ["paused", "blocked"].includes(job.status) ? <div className="inline-actions">{hvLink}<form action={resumeJob}><input type="hidden" name="jobId" value={job.id}/><input type="hidden" name="version" value={job.version}/><button className="button compact primary" type="submit">Resume to queue</button></form></div>
        ...
        return <tr key={job.id}><td><div className="cell-main">{job.description_snapshot}</div><div className="cell-sub mono">{job.operation_code ?? job.id.slice(0, 8)}</div>{job.rework_kind ? <div className="cell-sub">{job.rework_kind}: {job.rework_reason}</div> : null}</td><td className="mono">{job.repair_order?.ro_number}</td><td><StatusPill label={job.safety_class} tone={job.safety_class.includes("hv_") ? "red" : job.safety_class === "ev_aware" ? "amber" : "gray"} /></td><td><div className="cell-main mono">{job.planned_minutes}m / {Math.round(minutes)}m</div><div className="cell-sub">{job.required_qualification_code ?? "No qualification gate"}</div></td><td>{technician ? <><div className="cell-main">{technician.employee_no ?? `Tech ${technician.id.slice(0, 6)}`}</div><div className="cell-sub">{technician.labor_grade ?? "Technician"}</div></> : "Unassigned"}</td><td><StatusPill label={job.status} tone={statusTone(job.status)} /></td><td>{job.cause_text && job.correction_text ? <><div className="cell-main">Cause: {job.cause_text}</div><div className="cell-sub">Correction: {job.correction_text}</div></> : <form action={recordJobNarrative} className="stack"><input type="hidden" name="jobId" value={job.id}/><input name="causeText" aria-label="Cause" placeholder="Cause" required/><input name="correctionText" aria-label="Correction" placeholder="Correction" required/><button className="button compact" type="submit">Save narrative</button></form>}{latestInterruption ? <div className="cell-sub">{latestInterruption.interruption_type}: {latestInterruption.reason}</div> : null}{job.status === "completed" ? <form action={createReworkJob} className="stack"><input type="hidden" name="jobId" value={job.id}/><select name="reworkKind" aria-label="Rework type"><option value="rework">Rework</option><option value="comeback">Comeback</option></select><input name="reason" aria-label="Rework reason" placeholder="Required reason" required/><button className="button compact" type="submit">Create follow-up</button></form> : null}</td><td>{action}</td></tr>;
