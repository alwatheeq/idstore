...
import { RecordRemovalForm } from "@/components/record-removal-form"; import { discardDraft } from "@/app/(app)/records/manage/actions"; import Link from "next/link";
...
type PageQuery = { discard?: string; new?: string; line?: string; pay?: string; created?: string; error?: string };
...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  const discardable = staff.role === "admin" ? invoices?.find(record => record.id === query.discard && record.status === "draft") : undefined; return <LocalizedContent>{<>
    {discardable ? <RecordRemovalForm action={discardDraft} fields={{ kind: "invoice", id: discardable.id, updatedAt: discardable.updated_at }} title="Discard draft" name={discardable.repair_order?.ro_number ?? "Draft"} description="Discard this draft. Unposted invoice lines are deleted; estimate versions remain in history. This cannot be undone here." back="/invoices" /> : null}<PageHeader eyebrow="Billing & tax" title="Invoices" description="Build the service invoice, lock its commercial snapshot at posting, and allocate branch payments against the exact balance.">
    ...
    {selectedPayable ? <section className="panel operation-form" id="receive-payment">
      ...
      <form action={receivePayment} className="form-grid panel-body"><SubmissionKey />
