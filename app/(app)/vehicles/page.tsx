...
export default async function VehiclesPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  const [{ data: branches }, { data: customers }, { data: models }, { data: vehicles, error }, { data: recommendations }] = await Promise.all([
    ...
    supabase.from("vehicle_recommendations").select("vehicle_id, severity").eq("organization_id", staff.organizationId).eq("status", "open"),
  ]);
  ...
  const safetyAlerts = (recommendations ?? []).filter((item) => item.severity === "red" || item.severity === "safety_stop").length;
  ...
  return <>
    <PageHeader eyebrow="Vehicle registry" title="VW ID vehicles" description="VIN-led service history, ownership, odometer, battery health and technical diagnostics.">
      {setupReady ? <Link className="button primary" href="/vehicles?new=1#new-vehicle"><Plus /> Register vehicle</Link> : <Link className="button primary" href={branches?.length ? "/customers?new=1#new-customer" : "/branches?new=1#new-branch"}><Plus /> Complete setup</Link>}
    </PageHeader>
