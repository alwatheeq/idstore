...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  return <>
    ...
    <section className="dashboard-scope" aria-label="Dashboard branch scope">
      ...
      <form className="scope-form" action="/dashboard"><BranchField id="dashboard-branch" name="branch" label="Branch" branches={branches} selectedBranchId={staff.selectedBranchId} value={selectedBranchId} placeholder="All accessible branches" /><button className="button" type="submit">Apply scope</button></form>
    </section>
    <MetricStrip metrics={[
