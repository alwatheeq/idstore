...
export default async function SearchPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  return <><PageHeader eyebrow="Organization-wide lookup" title="Find any operational record" description="Search identifiers and names across every branch you are permitted to access." />
    <form className="global-search-hero" action="/search"><Search /><input name="q" defaultValue={q} placeholder="Customer, VIN, registration, RO, invoice, part or PO…" autoFocus /><button className="button primary" type="submit">Search</button></form>
