import { BarChart3, Boxes, CircleDollarSign, Clock3, ShieldAlert, Wrench } from "lucide-react";
...
export default async function ReportsPage() {
  ...
  const [{ data: branches }, { data: orders }, { data: appointments }, { data: balances }, { data: recommendations }, { data: invoices }, { data: payments }, { data: refunds }] = await Promise.all([
  ...
  const branchRows = (branches ?? []).map((branch) => {
    ...
    return {
      ...
      appointments: (appointments ?? []).filter((item) => item.branch_id === branch.id && ["booked", "confirmed", "checked_in"].includes(item.status)).length,
      ...
      safetyDeferred: (recommendations ?? []).filter((item) => item.branch_id === branch.id && item.severity === "safety" && !["completed", "dismissed"].includes(item.status)).length,
