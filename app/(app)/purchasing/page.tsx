...
type PageQuery = { new?: "supplier" | "order"; line?: string; receive?: string; invoice?: string; created?: string; error?: string };
...
export default async function PurchasingPage({ searchParams }: { searchParams: Promise<PageQuery> }) {
  ...
  const [{ data: branches }, { data: suppliers }, { data: parts }, { data: bins }, { data: orders, error }, { data: receipts }, { data: supplierInvoices }] = await Promise.all([
    ...
    supabase.from("supplier_invoices").select("id, supplier_invoice_no, invoice_date, grand_total, expected_total, variance_total, landed_cost_total, match_status, purchase_order:purchase_orders(po_number), supplier:suppliers(name), supplier_invoice_lines(id, invoiced_quantity, invoiced_unit_cost, expected_unit_cost, quantity_variance, price_variance, purchase_order_line:purchase_order_lines(part:parts(part_number)))").eq("organization_id", staff.organizationId).order("created_at", { ascending: false }).limit(25),
  ]);
  ...
  const invoiceOrder = orders?.find((order) => order.id === query.invoice);
  ...
  return <LocalizedContent>{<>
    ...
    {selectedLine && ["confirmed", "partially_received"].includes(selectedLine.order.status) ? <section className="panel operation-form" id="receive-line">
      ...
      <form action={receivePurchaseOrderLine} className="form-grid panel-body"><SubmissionKey />
    ...
    {invoiceOrder && ["partially_received","received","closed"].includes(invoiceOrder.status) && invoiceOrder.purchase_order_lines.length ? <section className="panel operation-form" id="supplier-invoice"><div className="panel-header"><div><div className="panel-title">Three-way supplier invoice match · {invoiceOrder.po_number}</div><div className="panel-subtitle">Compares ordered cost, received quantity and invoiced value; freight is allocated by receipt value.</div></div><Link className="panel-link" href="/purchasing">Cancel</Link></div><form action={recordSupplierInvoice} className="form-grid panel-body"><input type="hidden" name="purchaseOrderId" value={invoiceOrder.id}/><div className="form-field"><label htmlFor="supplier-invoice-no">Supplier invoice number</label><input className="mono" id="supplier-invoice-no" name="supplierInvoiceNo" required/></div><div className="form-field"><label htmlFor="supplier-invoice-date">Invoice date</label><input id="supplier-invoice-date" name="invoiceDate" type="date" required/></div><div className="form-field form-span-2"><label htmlFor="supplier-invoice-line">Purchase order line</label><select id="supplier-invoice-line" name="lineId" required>{invoiceOrder.purchase_order_lines.map(line=><option key={line.id} value={line.id}>#{line.line_no} · {line.part?.part_number} · received {Number(line.received_quantity).toLocaleString()}</option>)}</select></div><div className="form-field"><label htmlFor="supplier-invoice-quantity">Invoiced quantity</label><input id="supplier-invoice-quantity" name="quantity" type="number" min="0.001" step="0.001" required/></div><div className="form-field"><label htmlFor="supplier-invoice-cost">Invoiced unit cost</label><input id="supplier-invoice-cost" name="unitCost" type="number" min="0" step="0.001" required/></div><div className="form-field"><label htmlFor="landed-cost">Freight / landed cost</label><input id="landed-cost" name="landedCost" type="number" min="0" step="0.001" defaultValue="0"/></div><div className="form-field form-span-2"><label htmlFor="invoice-evidence">Evidence note</label><textarea id="invoice-evidence" name="evidenceNote" rows={2}/></div><div className="form-actions form-span-2"><button className="button primary" type="submit">Record and match</button></div></form></section> : null}

    <section className="panel"><div className="panel-header"><div className="panel-title">Suppliers</div>{staff.role === "admin" ? <RecordAction kind="archive" label="Archived records" href="/records/manage?kind=supplier" /> : null}</div><div className="data-scroll"><table className="data-table"><thead><tr><th>Supplier</th><th>Phone</th><th>Email</th>{staff.role === "admin" ? <th>Actions</th> : null}</tr></thead><tbody>{suppliers?.map(supplier => <tr key={supplier.id}><td>{supplier.name}</td><td><bdi dir="ltr">{supplier.phone ?? "—"}</bdi></td><td><bdi dir="ltr">{supplier.email ?? "—"}</bdi></td>{staff.role === "admin" ? <td className="record-action-cell"><MasterRecordActions kind="supplier" id={supplier.id} /></td> : null}</tr>)}</tbody></table></div></section><MetricStrip metrics={[
    ...
    {!orders?.length ? <EmptyState icon={ShoppingCart} title="No purchase orders" description={!branches?.length ? "Create a branch before ordering parts." : !parts?.length ? "Add catalog parts before ordering stock." : !suppliers?.length ? "Add the first supplier to start purchasing." : "Create a draft purchase order for the next supplier delivery."} action={!suppliers?.length ? <Link className="button primary" href="/purchasing?new=supplier#new-supplier">Add first supplier</Link> : setupReady ? <Link className="button primary" href="/purchasing?new=order#new-order">Create purchase order</Link> : undefined} /> : <div className="stack">
      <section className="panel"><div className="panel-header"><div><div className="panel-title">Purchase-order control</div><div className="panel-subtitle">Order state and commercial totals</div></div></div><div className="data-scroll"><table className="data-table purchase-order-ledger"><thead><tr><th>Purchase order</th><th>Supplier</th><th>Branch</th><th>Status rail</th><th className="align-right">Total</th><th>Next action</th></tr></thead><tbody>{orders.map((order) => {
        ...
        return <tr key={order.id}><td><div className="cell-main mono">{order.po_number}</div><div className="cell-sub">{order.purchase_order_lines.length} line{order.purchase_order_lines.length === 1 ? "" : "s"} · {order.ordered_at ? `sent ${dateTime.format(new Date(order.ordered_at))}` : "not submitted"}</div></td><td className="cell-main">{order.supplier?.name}</td><td>{order.branch?.city} · {order.branch?.code}</td><td><div className="po-state"><StatusPill label={order.status} tone={statusTone(order.status)} /><div className="po-rail" aria-label={`Purchase order status ${order.status}`}>{stages.slice(0, 5).map((stage, index) => <span className={order.status !== "cancelled" && index <= currentStage ? "complete" : ""} key={stage} />)}</div></div></td><td className="align-right"><div className="cell-main mono">{money.format(Number(order.grand_total))}</div><div className="cell-sub">Tax {money.format(Number(order.tax_total))}</div></td><td><div className="inline-actions">{order.status === "draft" ? <Link className="button compact" href={`/purchasing?line=${order.id}#add-line`}>Add line</Link> : null}{["partially_received","received","closed"].includes(order.status)?<Link className="button compact" href={`/purchasing?invoice=${order.id}#supplier-invoice`}>Match invoice</Link>:null}{purchaseOrderActions(order.status).map((action, index) => <form action={transitionPurchaseOrder} key={action.value}><input type="hidden" name="purchaseOrderId" value={order.id} /><input type="hidden" name="toStatus" value={action.value} /><button className={`button compact ${index === 0 ? "primary" : ""}`} type="submit">{action.label}</button></form>)}</div></td></tr>;
      ...
      <section className="panel"><div className="panel-header"><div><div className="panel-title">Supplier invoice reconciliation</div><div className="panel-subtitle">Ordered, received and invoiced evidence with landed-cost allocation</div></div></div><div className="data-scroll"><table className="data-table"><thead><tr><th>Invoice</th><th>Supplier / PO</th><th>Status</th><th className="align-right">Expected</th><th className="align-right">Invoiced</th><th className="align-right">Variance</th><th>Line evidence</th></tr></thead><tbody>{(supplierInvoices??[]).map(invoice=><tr key={invoice.id}><td><div className="cell-main mono">{invoice.supplier_invoice_no}</div><div className="cell-sub">{new Date(invoice.invoice_date).toLocaleDateString("en-JO")}</div></td><td><div className="cell-main">{invoice.supplier?.name}</div><div className="cell-sub mono">{invoice.purchase_order?.po_number}</div></td><td><StatusPill label={invoice.match_status} tone={invoice.match_status==="matched"?"green":invoice.match_status==="exception"?"red":"amber"}/></td><td className="align-right mono">{money.format(Number(invoice.expected_total))}</td><td className="align-right"><div className="cell-main mono">{money.format(Number(invoice.grand_total))}</div><div className="cell-sub">landed {money.format(Number(invoice.landed_cost_total))}</div></td><td className="align-right mono">{money.format(Number(invoice.variance_total))}</td><td>{invoice.supplier_invoice_lines.map(line=><div className="cell-sub" key={line.id}>{line.purchase_order_line?.part?.part_number}: qty Δ {Number(line.quantity_variance).toLocaleString()} · price Δ {money.format(Number(line.price_variance))}</div>)}</td></tr>)}{!supplierInvoices?.length?<tr><td colSpan={7}><div className="table-empty">No supplier invoices matched yet.</div></td></tr>:null}</tbody></table></div></section>
    </div>}
  </>}</LocalizedContent>;
}
