  ...
  return <>
    ...
    {query.new === "order" && setupReady ? <section className="panel operation-form" id="new-order">
      ...
      <form action={createPurchaseOrder} className="form-grid panel-body">
        <div className="form-field"><label htmlFor="po-branch">Receiving branch</label><select id="po-branch" name="branchId" required><option value="">Select branch</option>{branches?.map((branch) => <option key={branch.id} value={branch.id}>{branch.city} · {branch.code}</option>)}</select></div>
