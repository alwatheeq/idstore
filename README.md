...
The development Supabase project is live and all eleven versioned migrations have been applied. Mobile-number/6-digit-PIN authentication, the multi-branch shell, branch setup, customer creation, vehicle registration, appointment scheduling and status management, staff provisioning, branch/capability assignment, work-order intake, workshop dispatch, technician timers, parts catalog setup, supplier purchasing, lot/serial receiving, stock movements, service invoicing and payment allocation now read and write live Supabase data through typed, permission-checked commands. The staff Auth function is deployed with JWT verification, and the operational flows have been transaction-tested against the connected project.

Dashboard analytics still use prototype presentation data and are the next implementation phase. Invoice posting emits a durable outbox event, but the Jordan e-invoicing adapter remains pending official sandbox credentials and accountant-approved tax examples. GitHub synchronization still requires valid GitHub CLI authentication and a configured repository remote.
